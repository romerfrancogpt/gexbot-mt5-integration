...
//|                       FIX: eliminado parpadeo en refresh/drag    |
//+------------------------------------------------------------------+
...
//--- NOTA: WebRequest() es una llamada BLOQUEANTE en MQL5 (congela la
//--- terminal mientras espera respuesta). Se hacen 2 llamadas por ciclo
//--- (classic + state). Si el API responde lento (comun en la apertura
//--- de mercado por carga alta), la terminal puede quedar "congelada"
//--- hasta por 2 x InpWebRequestTimeoutMs. Bajar este valor reduce el
//--- tiempo maximo de bloqueo por ciclo.
...
void FetchAPIData()
  {
   // Guard anti-reentrada: si por alguna razon el timer se dispara mientras
   // una llamada anterior sigue en curso, se ignora en vez de apilarse.
   static bool fetching = false;
   if(fetching) { Print("Gexbot: ciclo anterior aun en curso, se omite este refresh."); return; }
   fetching = true;
   ...
   if(resClassic == 200) 
     ...
     {
      // Log para diagnosticar congelamientos: si esto se llena de timeouts
      // (error 5203/5204) justo en apertura de mercado, confirma que el API
      // esta respondiendo lento y por eso la terminal se traba.
      PrintFormat("Gexbot Classic API fallo. HTTP=%d GetLastError=%d URL=%s", resClassic, GetLastError(), urlClassic);
     }
   ...
   if(InpShowStateCenter)
     {
      ...
      if(resState == 200)
        ...
        {
         PrintFormat("Gexbot State API fallo. HTTP=%d GetLastError=%d URL=%s", resState, GetLastError(), urlState);
        }
     }
