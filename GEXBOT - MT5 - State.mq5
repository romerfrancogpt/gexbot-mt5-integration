...
//|        EA Classic (Sides) + State (Center)                       |
//|                       Round-Robin API Architecture               |
//+------------------------------------------------------------------+
...
input int           InpRefreshSeconds  = 3;                               // Ciclo de peticiones (Round-Robin) seg
input int           InpWebRequestTimeoutMs = 3000;                        // Timeout de conexion al API (ms)
...
input ENUM_STATE_GREEK InpStateGreek   = GREEK_GAMMA;                     // Greek a mostrar al centro
...
//--- Variables del Dashboard y Lógica
double ZeroGamma = 0;
double NetGexVol = 0;
double NetGexOI = 0;
...
int ApiCallStep = 0; // Rotación Round-Robin: 0=Classic, 1=State
...
int OnInit()
  {
   ...
   // Fetch inicial COMPLETO (Classic + State) para que al recompilar
   // por cambio de inputs (ej. DTE) no quede State esperando su turno
   // del round-robin. El reinicio completo en OnInit/OnDeinit se
   // mantiene intacto (evita el colgado de pantalla).
   FetchClassic();
   FetchState();
   ApiCallStep = 0; // el timer retoma el ciclo normal desde Classic
...
string GetGreekDteSuffix()
  {
   // El endpoint State solo acepta "zero" o "one". "full" no existe para
   // este endpoint, así que se usa "zero" como fallback (ver aviso en
   // GetGreekDisplayLabel para que quede visible en el panel).
...
string GetGreekDisplayLabel()
  {
   ...
   if(InpDte == DTE_full)
      return g + " (" + dte + " — full no soportado en State)";

   return g + " (" + dte + ")";
  }

//+------------------------------------------------------------------+
string BuildStateUrl()
  {
   string path = GetGreekKey() + "_" + GetGreekDteSuffix();
   return "https://api.gexbot.com/" + InpTicker + "/state/" + path + "?ts=" + IntegerToString(GetTickCount64());
  }
...
void FetchClassic()
  {
   ...
   int resClassic = WebRequest("GET", urlClassic, headers, InpWebRequestTimeoutMs, post, result, result_headers);
   if(resClassic == 200) ParseClassicAPIResponse(CharArrayToString(result));
  }

void FetchState()
  {
   if(!InpShowStateCenter) return;

   string headers = "Authorization: Bearer " + InpApiKey + "\r\nUser-Agent: MT5_Client/1.0\r\nAccept: application/json\r\nCache-Control: no-cache\r\n";
   char post[], result[]; string result_headers;

   string urlState = BuildStateUrl();
   int resState = WebRequest("GET", urlState, headers, InpWebRequestTimeoutMs, post, result, result_headers);
   if(resState == 200) ParseStateAPIResponse(CharArrayToString(result));
  }
...
void FetchAPIData()
  {
   ...
   // Lógica Round-Robin: Cada ejecución del timer llama a UN solo endpoint
   if(ApiCallStep == 0)
     {
      FetchClassic();
      ApiCallStep++;
     }
   else if(ApiCallStep == 1)
     {
      FetchState();
      ApiCallStep = 0; // Reinicia el ciclo
     }
   ...
   for(int i = total - 1; i >= 0; i--)
     {
      ...
      bool found = false;
      for(int j = 0; j < ArraySize(validNames); j++) { if(validNames[j] == name) { found = true; break; } }
   ...
   while(pos < StringLen(json))
     {
      ...
      if(ArraySize(vals) >= 4)
        {
         double strk = StringToDouble(vals[0]) * InpStrikeMultiply;
         double greekVal = StringToDouble(vals[3]); 
